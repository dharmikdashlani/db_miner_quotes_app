...
class _ConfidenceState extends State<Confidence> {
  ...
  fetch() async {
    Future.delayed(
        ...
        () => setState(() {
              getAllQuotes = DBHelper.dbHelper.fetachAllConfidenceRecords();
            }));
  }

  forGetVariable() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    bool? isConfidenceArrived = prefs.getBool('confidenceArrived') ?? false;

    (isConfidenceArrived == false)
        ? DBHelper.dbHelper.lodeConfidenceJasonBank()
        : getAllQuotes = DBHelper.dbHelper.fetachAllConfidenceRecords();
  }
  ...
  isDataArrived() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setBool('confidenceArrived', true);
  }
